...
for (var i = 2; i < nodeArgs.length; i++) {
    ...
    if (i > 2 && i < nodeArgs.length) {
        actorsName = actorsName + "+" + nodeArgs[i];
    }
    else {
        actorsName += nodeArgs[i];
    }
}
var queryUrl = "http://www.omdbapi.com/?t=" + actorsName + "&y=&plot=short&apikey=trilogy";

axios.get(queryUrl).then(
    function (response) {
        console.log("Movie: " + response.data.Actors);
        fs.appendFile('log.txt', "Movie: " + response.data.Actors + '\n', function (err) {
            if (err) throw err;
        });
